...
function auth(_username, _token) {
  gameSocket.emit('auth', { username: _username , token: _token } , function(){
    selectMap();
    getSide();
    getUnits();
  });
...
function selectMap() {
  gameSocket.emit('select-map', null, function(data) {
    route_map += data;
...
function getSide() {
  gameSocket.emit('get-side', null, function(data) {
    if(data === true)
      left = true;
    else
      left = false;
...
function getUnits() {
  gameSocket.emit('get-units', null, function(data) {
    units = data;
...
{ "sprite": "july_perso", "taille": "petitfin", "name": "July?", "statut": -1, "life": 100 },
];
